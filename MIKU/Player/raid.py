...
async def spam(e):
    ...
    if e.sender_id in SUDO_USERS:
        ...
        if len(Deadly) == 2:
            ...
            g = a.id
            if int(g) in Deadly:
                text = f"I can't raid on VOID NETWORK's Owner"
            ...
            if int(g) in DEADLYSPAM:
                text = f"I can't raid on @Miku_Support's Owner"
                await e.reply(text, parse_mode=None, link_preview=None )
            elif int(g) == OWNER_ID:
                text = f"This Dude is owner Of this Bots."
                await e.reply(text, parse_mode=None, link_preview=None )
    ...
    if e.sender_id in SUDO_USERS:
        ...
        if len(e.text) > 11:
            ...
            if int(user_id) in Deadly:
                text = f" can't raid on @Miku_Support's Owner."
                await e.reply(text, parse_mode=None, link_preview=None )
            ...
            if int(user_id) in DEADLYSPAM:
                ...
                text = f"This Dude is owner Of this Bot."
                await event.reply(text, parse_mode=None, link_preview=None )
            elif int(user_id) in SUDO_USERS:
                text = f"This Dude is sudo user."
                await e.reply(text, parse_mode=None, link_preview=None )
